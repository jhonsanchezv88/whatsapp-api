...
| - | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| 1 | ¿Railway soporta volúmenes persistentes para `/evolution/instances/`?                                                      | Sí. Railway soporta volúmenes persistentes y permite montarlos en una ruta específica dentro del servicio. Para Evolution API, el volumen suele montarse en `/evolution/instances`, ya que allí se almacenan datos asociados a las sesiones de WhatsApp. Sin embargo, existe una limitación importante: Railway no permite usar réplicas nativas en servicios que tienen volúmenes montados. Por esta razón, si Evolution API depende de ese volumen, no conviene escalar el mismo servicio con el control de réplicas de Railway.                                                                     | Respondida                                                |
| 2 | ¿Cuántas sesiones puede manejar una réplica de Evolution API con 2 GB de RAM en Railway?                                   | No existe un número oficial exacto, porque depende del tráfico de cada sesión, cantidad de mensajes, archivos multimedia, reconexiones, eventos y consumo real de Baileys. Como estimación conservadora para el proyecto, una réplica de 2 GB puede planearse inicialmente para un rango aproximado de 50 a 100 sesiones activas. Para validar este dato, se recomienda realizar pruebas graduales con 10, 25, 50, 75 y 100 sesiones, midiendo consumo de RAM, CPU, latencia, reconexiones y estabilidad.                                                                                              | Respondida, requiere prueba de carga                      |
| 3 | ¿Evolution API soporta un campo `cleanedSenderPn` o se debe parsear el JID manualmente?                                    | No se debe asumir que Evolution API entregue un campo equivalente a `cleanedSenderPn` de Wasender. En Evolution API, lo habitual es obtener el identificador desde `data.key.remoteJid`, por ejemplo `573001112233@s.whatsapp.net`, y extraer el número removiendo el sufijo correspondiente. También debe tenerse en cuenta que WhatsApp está migrando algunos identificadores hacia formato `@lid`, por lo que no siempre será posible obtener directamente el número telefónico desde el webhook.                                                                                                   | Respondida                                                |
| 4 | ¿Se puede configurar una URL de webhook diferente por instancia?                                                           | Sí. Evolution API permite configurar webhooks por instancia. Esto puede hacerse al crear la instancia, enviando la configuración de webhook dentro del payload, o posteriormente mediante los endpoints de configuración de webhook. Esta opción permite que cada sesión tenga una URL propia y una lista específica de eventos habilitados.                                                                                                                                                                                                                                                           | Respondida                                                |
| 5 | ¿Cómo maneja Evolution API los WhatsApp LIDs frente a los números tradicionales?                                           | Evolution API depende de Baileys para la conexión con WhatsApp Web. Actualmente WhatsApp maneja identificadores tradicionales tipo PN, por ejemplo `573001112233@s.whatsapp.net`, y también identificadores tipo LID. Por esta razón, Logicfy no debería depender únicamente del número telefónico como identificador principal. Lo recomendable es almacenar tanto el identificador tradicional como el LID cuando estén disponibles, para evitar problemas futuros con eventos, mensajes o contactos que lleguen con identificadores distintos.                                                      | Respondida                                                |
| 6 | ¿Es posible obtener media, imagen o audio, directamente como URL pública desde Evolution API sin desencriptar manualmente? | Sí, siempre que Evolution API esté configurado con almacenamiento compatible con S3, como Cloudflare R2. En ese escenario, Evolution API puede recibir el archivo multimedia, subirlo al bucket configurado y entregar en el webhook una URL pública o accesible del archivo. De esta forma, Logicfy no tendría que desencriptar manualmente los archivos de WhatsApp, sino consumir la URL generada por Evolution API.                                                                                                                                                                                | Respondida                                                |
| 7 | ¿El costo mensual en Railway para 1.000 sesiones es competitivo frente a Wasender?                                         | Puede ser competitivo, pero depende del consumo real de recursos. Para 1.000 sesiones, el PRD estima que podrían requerirse entre 10 y 20 réplicas de Evolution API, además de PostgreSQL, Redis, RabbitMQ, almacenamiento persistente y tráfico de red. Aunque el costo de infraestructura propia en Railway podría ser menor que pagar una tarifa mensual por cada sesión en un servicio externo, es necesario validar el consumo real mediante pruebas de carga. La estimación final debe considerar RAM, CPU, base de datos, Redis, RabbitMQ, volumen, tráfico y costos asociados a Cloudflare R2. | Respondida, requiere estimación final con métricas reales |

### Resumen técnico

La mayoría de preguntas quedan respondidas técnicamente. El punto más delicado es el uso de volúmenes persistentes junto con réplicas en Railway. Si Evolution API necesita almacenar datos en `/evolution/instances`, no es recomendable escalar ese mismo servicio usando réplicas nativas de Railway.

Para la fase actual, la configuración más estable es:

```txt
Evolution API: 1 réplica estable
PostgreSQL: activo
Redis: activo
RabbitMQ: activo
Cloudflare R2: activo
Cron R2 Cleaner: activo
```

RabbitMQ ya permite avanzar en el manejo asincrónico de eventos y webhooks. Sin embargo, para escalar Evolution API con varias réplicas será necesario resolver la afinidad de sesión, ya sea con un gateway por instancia, servicios Evolution separados o una arquitectura más avanzada fuera de Railway si el volumen de sesiones crece demasiado.
